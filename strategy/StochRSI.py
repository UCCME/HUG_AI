...
# 伪代码示例
def stoch_rsi(close_prices, period=14, k_period=3, d_period=3):
    """
    计算Stochastic RSI
    """
    ...
    # 1. 计算RSI
    rsi = talib.RSI(close_prices, period)
    ...
    return stoch_rsi_k.values, stoch_rsi_d.values
...
def fisher_transform(high, low, period=10):
    ...
    normalized = 0.66 * ((median_price - min_price)
                         / (max_price - min_price) - 0.5)
...
def run_lucy_strategy(data
        ...
        data['zlsma'] = zlsma_.values if isinstance(zlsma_, pd.Series) else zlsma_ 
        
        # 计算UT Bot追踪止损线
        ut_trailing_stop = ut_bot_trailing_stop(close, high, low, volume, ut_atr_period, ut_key)
        data['ut_trailing_stop'] = ut_trailing_stop if isinstance(ut_trailing_stop, pd.Series) else ut_trailing_stop
        ...
        # Stoch RSI + Fisher 
        stoch_rsi_k, stoch_rsi_d = stoch_rsi(close, 14, 3)
        fisher, fisher_signal = fisher_transform(high, low)
        stoch_crossover = _crossover(stoch_rsi_k, stoch_rsi_d)
        stoch_crossunder = _crossunder(stoch_rsi_k, stoch_rsi_d)
        fisher_crossover = _crossover(fisher, fisher_signal)
        fisher_crossunder = _crossunder(fisher, fisher_signal)
        data['stoch_rsi_k'] = stoch_rsi_k
        data['stoch_rsi_d'] = stoch_rsi_d
        data['fisher'] = fisher
        data['fisher_signal'] = fisher_signal
        data['stoch_crossover'] = stoch_crossover if isinstance(stoch_crossover, pd.Series) else stoch_crossover
        data['stoch_crossunder'] = stoch_crossunder if isinstance(stoch_crossunder, pd.Series) else stoch_crossunder
        # stoch_buy = self.stoch_buy[-1] and self.stoch_rsi_k[-1] < 20
        # stoch_sell = self.stoch_sell[-1] and self.stoch_rsi_k[-1] > 80
        data['stoch_buy_signal'] = data['stoch_crossover'] & (data['stoch_rsi_k'] < stoch_rsi_oversold)
        data['stoch_sell_signal'] = data['stoch_crossunder'] & (data['stoch_rsi_k'] > stoch_rsi_overbought)
        data['fisher_buy_signal'] = fisher_crossover if isinstance(fisher_crossover, pd.Series) else False
        data['fisher_sell_signal'] = fisher_crossunder if isinstance(fisher_crossunder, pd.Series) else False
...
class StochRSIFisher(Strategy):
    ...
    def init(self):
        ...
        # Stoch RSI + Fisher 
        self.stoch_rsi_k, self.stoch_rsi_d = self.I(stoch_rsi, close, 14, 3)
        self.fisher, self.fisher_signal = self.I(fisher_transform, high, low)
        self.stoch_buy = self.I(_crossover, self.stoch_rsi_k, self.stoch_rsi_d)
        self.stoch_sell = self.I(_crossunder, self.stoch_rsi_k, self.stoch_rsi_d)
        self.fisher_buy = self.I(_crossover, self.fisher, self.fisher_signal)
        self.fisher_sell = self.I(_crossunder, self.fisher, self.fisher_signal)
    ...
    def next(self):
        ...
        # === 最终交易决策 ===
        # 买入条件：基础多头信号 且 UT买入信号
        # print(self.stoch_buy[-1], self.stoch_rsi_k[-1])
        stoch_buy = self.stoch_buy[-1] and self.stoch_rsi_k[-1] < 20
        stoch_sell = self.stoch_sell[-1] and self.stoch_rsi_k[-1] > 80
        fisher_buy = self.fisher_buy[-1]
        fisher_sell = self.fisher_sell[-1]

        buy_condition = stoch_buy & fisher_buy
        
        # 卖出条件：基础空头信号 且 UT卖出信号
        sell_condition = stoch_sell & fisher_sell 
